...
function run(self, name, count)
   ...
   if not self.silent then 
	 ...
	 if err:find("scape.test.unit") then
	    stat = "FAILED"
	 else
	    stat = "ERROR"
	    _print(err)
...
function assert(assertion)
   _assert(assertion,"scape.test.unit")
end

--- Fail test.
function fail()
   _assert(false,"scape.test.unit")
end
